...
Event *ev_create(char *name, char *eventData, unsigned long nAUs) {
   ...
   Event *t = (Event *)mem_alloc(sizeof(Event));
   if (t) {
      t->data = str_dupl(eventData);
      if (t->data) {
         t->topic = str_dupl(name);
         if (t->topic) {
            t->refCount = nAUs;
            (void) top_schema(name, &ncols, &schema);
            d = (DataStackEntry *)mem_alloc(ncols * sizeof(DataStackEntry));
            if (d) {
            ...
            mem_free((void *)(t->topic));
         }
         mem_free((void *)(t->data));
      }
      mem_free((void *)t);
   }
   return NULL;
...
void ev_release(Event *event) {
   ...
   if (event->refCount == 0) {
      //printf("%p - freeing\n", event);
      mem_free((void *)event->data);
      mem_free((void *)event->topic);
      mem_free((void *)event->theData);
      mem_free((void *)event);
   }
   pthread_mutex_unlock(&ev_mutex);
}
