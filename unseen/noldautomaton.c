 ...
 */
   //HIDE fprintf(stderr, "automaton %lu exiting\n", self->id);
   self->has_exited++;
/*
 * return storage associated with topic->variable mapping
 */
   //HIDE fprintf(stderr, "returning storage for topic->variable mapping\n");
 ...
 */
   //HIDE fprintf(stderr, "no longer registered for any topics, release events\n");
 ...
 */
   //goto label;
   //HIDE fprintf(stderr, "remove variables hash table\n");
   n = tsht_keys(self->variables, &keys);
   for (i = 0; i < n; i++) {
      DataStackEntry *dx;
      //HIDE fprintf(stderr, "  variable %s: ", keys[i]);
      tsht_remove(self->variables, keys[i], (void **)&dx);
      //HIDE dumpDataStackEntry(dx, 1);
      if (dx->type == dEVENT)
 ...
 */
   //HIDE fprintf(stderr, "free init and behav code sequences\n");
   mem_free(self->init);
   mem_free(self->behav);
 ...
 */
   //HIDE fprintf(stderr, "disconnect rpc binding\n");
   if (execerr) {
      char *s = (char *)pthread_getspecific(execerr_key);
      Q_Decl(buf,1024);
      char resp[100];
      unsigned rlen, len;
      //HIDE fprintf(stderr, "first send RPC to indicate execution error\n");
   ...
   if (au) {
	 ...
	 pthread_mutex_lock(&compile_lock);
	    ...
	    a_init();
	       ...
	       //HIDE dumpCompilationResults(au->id, au->variables, au->init, initSize, au->behav, behavSize);
               au->topics = topics;
