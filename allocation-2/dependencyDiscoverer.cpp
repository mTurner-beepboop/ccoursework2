// Alex Paterson, 2382854p, SP Exercise 2
 ...
 * set of directories separated by ':' to find included filesDOX
...
// thread safe structure for workQ
struct struct_workQ {     
  private:
    std::list<std::string> workQ; 
    std::mutex mutex; // mutex to protect critical section
  public:
    ...
    int size(std::list<std::string> workQ) {
      std::unique_lock<std::mutex> lock(mutex);
      return workQ.size();
    }
    ...
    std::string pop_front() {
      std::unique_lock<std::mutex> lock(mutex);
      if (workQ.size() > 0) {     // if workQ is not empty
        auto v = workQ.front();
        workQ.pop_front();
        return v;
      }
      return "";                  // if workQ is empty, return empty string
    }
};

// thread safe structure for theTable
struct struct_theTable {
  private:
    std::unordered_map<std::string, std::list<std::string>> theTable;
    std::mutex mutex; // mutex to protect critical section
  public:
    std::unordered_map<std::string,std::list<std::string>>::iterator find(std::string x) {
      std::unique_lock<std::mutex> lock(mutex);
      return theTable.find(x);
    }
...
struct_theTable s_theTable;
struct_workQ s_workQ;
...
static FILE *openFile(std::string file) {
  FILE *fd;
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    process_mutex.lock();   // prevent interference from different threads
    //std::thread::id this_id = std::this_thread::get_id();   //if you want to see what threads are currently running, uncomment these two lines!
    //std::cout << "thread " << this_id << " going...\n";
    if (s_theTable.find(name) != s_theTable.end()) { 
      process_mutex.unlock();
      continue; 
    }
    // ... insert mapping from file name to empty list in table ...
    s_theTable.insert({name, {}});
    // ... append file name to workQ
    s_workQ.push_back( name );
    process_mutex.unlock();
  }
  // 3. close file
  fclose(fd);
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = s_theTable.get(name);
...
int main(int argc, char *argv[]) {
  ...
  // look up CRAWLER_THREADS in environment
  auto crawls = getenv("CRAWLER_THREADS");
  int CRAWLER_THREADS = 2;  // if null, default to 2
  if (crawls != NULL) {   
    CRAWLER_THREADS = std::stoi(crawls);
  }
  ...
  for (i = start; i < argc; i++) {
    ...
    s_theTable.insert( { obj, { argv[i] } } );
    
    // 3b. insert mapping from file.ext to empty list
    s_theTable.insert( { argv[i], { } } );
    
    // 3c. append file.ext on workQ
    s_workQ.push_back( argv[i] );
  }

  // 4. for each file on the workQ
  std::vector<std::thread> ts;    // create vector of threads

  for (int i = 0; i < CRAWLER_THREADS; i++) {   // generate and control each thread
    ts.emplace_back([]() {
      std::string filename = s_workQ.pop_front();

      while (filename != "") {    // if pop_front() returns "", that means that s_workQ is empty (size is equal to 0) and this thread shall stop working on the current file
        if (s_theTable.find(filename) == s_theTable.end()) {
          fprintf(stderr, "Mismatch between table and workQ\n");
          return -1;
        }

        // 4a&b. lookup dependencies and invoke 'process'  
        process(filename.c_str(), s_theTable.get(filename));
        filename = s_workQ.pop_front();   // get next file in s_workQ
      }
      return 0;
    });
  }
