/*
 * James Campbell 2398110c, SP Exercise 2
...
public:
    ...
    std::optional<std::string> pop_front(){
        std::unique_lock<std::mutex> lock(mutex);
        if (items.size() > 0) {
            std::optional<std::string> returnval = items.front();
            items.pop_front();
            return returnval;
        } else{
            return {};
...
public:
    void insert( std::pair<std::string, std::list<std::string>> ilist){
        std::unique_lock<std::mutex> lock(mutex);
        table.insert(ilist);
    }
    ...
    auto findElseInsert(std::string key, std::pair<std::string, std::list<std::string>> ilist){
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    ll->push_back( {name} );
    // 2bii. if file name not already in table ...insert mapping from file name to empty list in table
    if (theTable.findElseInsert(name, { name, {} })) { continue; }
...
void worker(){
    ...
    while ( auto filename = workQ.pop_front()) {
...
int main(int argc, char *argv[]) {
  ...
  // 4. Now with threads
  // now that everything is set up, create threads and use them to run the worker process
  std::vector<std::thread> threads;
  int numThreads;

  if( std::getenv("CRAWLER_THREADS") ){
      std::string threadString = std::getenv("CRAWLER_THREADS");
      // make sure there's only numeric digits
      if(threadString.find_first_not_of( "0123456789" ) == std::string::npos) {
          numThreads = std::stoi(threadString);
          // turn 0 into 2 as we don't want 0 threads
          if (numThreads == 0 ){
              numThreads = 2;
          }
      } else{
