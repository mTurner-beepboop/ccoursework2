...
//Mark Turner, 2386300t, SP Exercise 2
//This is my own work, as defined in the ethics agreement, of course excluding that which was provided to be altered, ie the original dependency dicoverer file
...
struct workQ{
  ...
  public:
    ...
    std::string pop(){ //Call will never be made if list is empty as a check is performed before the call
      std::unique_lock<std::mutex> lock(mutex);
...
struct theTable{
  ...
  public:
    ...
    void insert_empty(std::string name){
      std::unique_lock<std::mutex> lock(mutex);
      theTable.insert( { name, {} } );
    }
    void insert(std::string name, std::string arg){
      std::unique_lock<std::mutex> lock(mutex);
      theTable.insert( { name, { arg } } );
    }
    std::list<std::string>* get_dependencies(std::string name){
      std::unique_lock<std::mutex> lock(mutex);
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = theTable.get_dependencies(name);
...
int main(int argc, char *argv[]) {
  ...
  for (i = start; i < argc; i++) {
    ...
    theTable.insert_empty(argv[i]);
    
    // 3c. append file.ext on workQ
    workQ.push( argv[i] );
  }

  // 4. for each file on the workQ    -- This is what has been altered to run using threads --
  //Set number of threads to run, 2 if thread_crawler is not set
  int thread_num;
  if (const char* CRAWLER_THREADS = std::getenv("CRAWLER_THREADS")){
    if (std::stoi(CRAWLER_THREADS) > 0){
      thread_num = std::stoi(CRAWLER_THREADS);
    } else { thread_num = 2; }
  } else {
    thread_num = 2;
  }
  
  //Create list of threads and populate with required amount, and set running
  std::list<std::thread> threads;

  for (int i = 0; i < thread_num; i++){
    threads.push_front( std::thread([](){
    while ( workQ.size() > 0 ) {
      ...
      process(filename.c_str(), theTable.get_dependencies(filename));
    }
    return 1; //Thread has terminated at end of queue  
    }) );

  }
  
  //Join all threads in the list once done
  for (auto& t: threads){
    t.join();
  }
